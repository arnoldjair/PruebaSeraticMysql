--Tabla propietario de los datos
create table user (
  id integer not null,
  primary key(id)
);
...
alter table categories add constraint categories_owner foreign key(owner) references user(id) on delete set null;

alter table movies add constraint categories_owner foreign key(owner) references user(id) on delete set null;

alter table movie_rentals add constraint categories_owner foreign key(owner) references user(id) on delete set null;

alter table members add constraint categories_owner foreign key(owner) references user(id) on delete set null;

alter table payments add constraint categories_owner foreign key(owner) references user(id) on delete set null;